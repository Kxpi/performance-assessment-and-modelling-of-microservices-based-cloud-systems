...
def percendacne_avg_time(groups, traces):
    percendance_time = {}
    print(groups)
    for group in groups:
        for trace in traces:

            if trace["traceID"] in group:

                sorted_spans = sorted(trace['spans'], key=lambda span: span['startTime'])
                for i, span in enumerate(sorted_spans[:-1]):
                    min_transfer = float('inf')
                    min_spanID = None
                    for span2 in sorted_spans[i+1:]:

                        if span['references'] and span2['references'] and span['references'][0]['spanID'] != span2['references'][0]['spanID']:

                            min_transfer = span2['startTime'] - span['startTime'] + span['duration']
                            min_spanID = span2['spanID']
                    if min_spanID is not None:
                        percendance_time[span['spanID']] = min_spanID
    return percendance_time
...
groups_info = get_groups(traces)
groups=[]
for part_groups in groups_info:
    groups.append(part_groups["traces"])

print(groups)

precedence_graph = percendacne_avg_time(groups,traces)
