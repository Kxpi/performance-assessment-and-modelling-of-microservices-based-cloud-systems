...
const TraceGraph = ({ selectedTrace, operationStats, serviceColors, selectedSpan, setSelectedSpan }) => {
    ...
    spans.forEach((span) => {
        initialNodes.push({
            ...
            selected: true ? span.spanID === selectedSpan : false
            ...
            <div style={{ height: '100%', width: '100%', display: "flex", flexDirection: "row", justifyContent: "space-around" }}>
                <ReactFlow
                    ...
                    onNodeClick={(event, node) => {
                        if (node) {
                            setSelectedSpan(node.data.spanID);
                ...
                {selectedSpan && <GraphInfo operationStats={operationStats} span={selectedTrace["spans"].find((span) => span.spanID === selectedSpan)} />}
            </div>
        </div >
    );
};
